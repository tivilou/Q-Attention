...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a spectral projector from relation anchor key vectors.")
    ...
    parser.add_argument("--output_path", default=None, help="Projector .pt path; defaults to <model_dir>/relation_projector.pt")
    ...
    parser.add_argument("--center", action="store_true", help="Mean-center anchor keys before SVD")
    parser.add_argument("--max_vectors", type=int, default=None, help="Optional deterministic subsample limit")
...
def metadata_path_for(output_path: Path) -> Path:
    ...
    return output_path.with_name(f"{output_path.stem}_metadata{suffix if suffix == '.json' else '.json'}")
...
def main() -> None:
    ...
    output_path = Path(args.output_path) if args.output_path else model_dir / "relation_projector.pt"
    ...
    projector = build_anchor_projector(collection.keys, projector_config, center=args.center)
    ...
    torch.save({"projector": projector.cpu(), "metadata": metadata}, output_path)
