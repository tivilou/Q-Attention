...
from q_attention.adapters import AttentionContextKernelAdapter  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train one Q-PVG value/context selector under a frozen relation contract."
    )
    ...
    parser.add_argument("--output_root", default="runs/q_pvg_formal_single_seed")
    ...
    parser.add_argument("--score_gain", type=float, default=0.25)
    parser.add_argument("--value_route_mode", default="branch_interpolation")
    parser.add_argument("--score_mode", default="value_only")
    parser.add_argument("--readout_mode", default="quantum")
    parser.add_argument("--phase_mode", default="complex")
...
def build_kernel(
    ...
    *,
    ...
    values = {
        key: value
        for key, value in dict(config_override or {}).items()
        if key in allowed_config
    }
    phase_mode = {
        "q_pvg_phase_value": "complex",
        "q_pvg_real_only": "real_only",
        "q_pvg_classical_complex": "complex",
        "q_pvg_random_phase": "random",
        "q_pvg_score_value": "complex",
    }[selector]
    readout_mode = "classical" if selector == "q_pvg_classical_complex" else "quantum"
    score_mode = "score_value" if selector == "q_pvg_score_value" else "value_only"
    values.update(
        {
            "num_layers": model_config.num_layers,
            "num_heads": model_config.num_heads,
            "head_dim": model_config.dim // model_config.num_heads,
            "register_qubits": int(getattr(args, "register_qubits", 2)),
            "depth": int(getattr(args, "depth", 2)),
            "angle_scale": float(getattr(args, "angle_scale", 1.0)),
            "gate_temperature": float(getattr(args, "gate_temperature", 1.0)),
            "score_gain": float(getattr(args, "score_gain", 0.25)),
            "value_route_mode": str(getattr(args, "value_route_mode", "branch_interpolation")),
            "phase_mode": phase_mode,
            "readout_mode": readout_mode,
            "score_mode": score_mode,
            "seed": seed + int(values.get("seed_offset", 3000)),
        }
    )
...
def context_hook_config(batch: Mapping[str, torch.Tensor], *, capture_trace: bool = False) -> Any:
    from q_attention.adapters.attention_context import AttentionContextHookConfig

    return AttentionContextHookConfig(
...
def evaluate(
    ...
    *,
    ...
    if kernel is not None:
        kernel.eval()
    query_chunk_size = getattr(kernel, "query_chunk_size", None) if kernel is not None else None
    adapter = None if kernel is None else AttentionContextKernelAdapter(
        model, model.context_module_paths, kernel, query_chunk_size=query_chunk_size
    )
    ...
    layer_accumulators = [
        ...
        for _ in model.context_module_paths
    ]
        ...
        for batch_index, raw_batch in enumerate(batches, start=1):
            ...
            try:
                if adapter is not None:
                    adapter.attach(context_hook_config(batch))
                    if collect_geometry:
                        for layer_index, path in enumerate(model.context_module_paths):
...
def train_kernel(
    ...
    adapter = AttentionContextKernelAdapter(
        ...
        kernel,
        query_chunk_size=getattr(args, "query_chunk_size", None),
    )
        ...
        while cursor.epoch <= args.epochs:
            ...
            for raw_batch in tracked_batches(
                ...
                for micro_index in range(micro_count):
                    ...
                    adapter.attach(context_hook_config(micro))
                    try:
                        logits = model(
                            micro["input_ids"],
