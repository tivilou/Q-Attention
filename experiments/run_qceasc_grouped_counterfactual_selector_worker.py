...
from run_retacred_qceasc_grouped_counterfactual_formal_single_seed import (  # noqa: E402
    CUDA_OOM_EXIT_CODE,
    ...
    *,
        ...
        for split in ("train", "valid", "test"):
            ...
            for local_index, record_index in enumerate(_indices_for(split)):
                ...
                if captures["counterfactual"]:
                    ...
                    for rep_id, key, axes in (
                        ("q_ceasc_auxiliary_state", "auxiliary_state", ["layers", "heads", "query_tokens", "auxiliary_state"]),
                        ("q_ceasc_observable_coefficients", "coefficients", ["layers", "heads", "query_tokens", "coefficients"]),
                        ("q_ceasc_projected_residual", "residual", ["layers", "heads", "query_tokens", "key_tokens"]),
                        ("q_ceasc_group_manifest", "group_manifest", ["layers", "heads", "query_tokens", "group_tokens"]),
                        ("q_ceasc_group_masked_supports", "group_masked_supports", ["layers", "heads", "query_tokens", "group_tokens", "support_dim"]),
                        ("q_ceasc_group_influence_vectors", "group_influence_vectors", ["layers", "heads", "query_tokens", "group_tokens", "support_dim"]),
                        ("q_ceasc_group_scores", "group_scores", ["layers", "heads", "query_tokens", "group_tokens"]),
                        ("q_ceasc_member_scores", "member_scores", ["layers", "heads", "query_tokens", "key_tokens"]),
                    ):
                        rows = []
                        for layer_index in range(int(model.config.num_layers)):
                            heads = []
    ...
    payload = {
        ...
        "provenance": {
            ...
            "plugin_sha256": _sha256(ROOT / "src" / "q_attention" / "plugins" / "q_ceasc_grouped_counterfactual.py"),
    ...
    sample_trace = {
        ...
        "experiment": {"run_id": output_dir.parent.parent.name, "dataset": "retacred.train+valid+test", "code_revision": _git_revision(), "config_sha256": _sha256(config_path), "model_identity": "relation-transformer-q-ceasc-grouped-counterfactual", "seed": int(config["seed"])},
