#!/usr/bin/env python3
"""Run one complete Re-TACRED Q-RPEC seed under the frozen contract."""
...
from q_attention.plugins.q_relation_perturbation_echo_curvature import (  # noqa: E402
...
DEFAULT_CONFIG = ROOT / "configs" / "retacred_qrpec_formal_single_seed.json"
...
def _source_contract() -> dict[str, Any]:
    paths = {
        "runner": ROOT / "experiments" / "run_retacred_qrpec_formal_single_seed.py",
        "worker": ROOT / "experiments" / "run_qrpec_selector_worker.py",
        ...
        / "q_attention"
        / "plugins"
        / "q_relation_perturbation_echo_curvature.py",
    }
    return {
...
def _validate_legacy_run_metadata(
    ...
    """Validate optional metadata emitted by the old Q-RPEC runner."""
...
def _render_selector_dashboard(
    ...
    lines = [
        (
            f"Q-RPEC selectors: {counts['complete']}/{total} complete | "
    ...
    try:
        while pending or active:
            ...
            while pending and available and not (pause is not None and pause.requested):
                ...
                command = [
                    args.python_bin,
                    str(ROOT / "experiments" / "run_qrpec_selector_worker.py"),
...
def build_kernel(
    ...
    *,
    ...
    mode_map = {"q_rpec": "quantum", "classical_local_echo": "local_control"}
    if mode not in mode_map:
        raise ValueError(f"unknown Q-RPEC selector {mode!r}")
    config_obj = RelationPerturbationEchoConfig(
...
def evaluate_selector(
    ...
    device: torch.device,
    label_count: int,
    kernel: RelationPerturbationEchoCurvatureKernel | None,
    stage: str,
) -> dict[str, Any]:
    return evaluate(
        model,
...
def _run(args: argparse.Namespace, pause: PauseController) -> int:
    ...
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if config.get("schema_version") != "q-attention.qrpec-formal-single-seed.v1":
        raise ValueError("unsupported Q-RPEC formal config")
    ...
    if model_parallel_gpu_ids:
        raise ValueError(
            "Q-RPEC formal handoff supports serial or selector-parallel GPUs only; "
    ...
    run_dir = args.resume or args.output_dir or ROOT / "runs" / "retacred_qrpec_formal_single_seed" / f"{provisional_stamp}_seed13"
    ...
    if args.import_baseline_from is not None:
        ...
        print(
            "[q-rpec] imported completed legacy baseline; "
            "q_rpec and control will restart from batch 0",
            flush=True,
        )
    ...
    summary = {
        "schema_version": "q-attention.qrpec-formal-single-seed.run.v1",
        "name": config["name"],
        "formal_experiment": True,
...
def main() -> int:
    ...
    try:
        ...
        print(f"[q-rpec] safely paused: {exc}", flush=True)
