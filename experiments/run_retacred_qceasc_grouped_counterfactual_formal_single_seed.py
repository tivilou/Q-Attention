#!/usr/bin/env python3
"""Run one complete Re-TACRED Q-CEASC seed under a frozen contract."""
...
DEFAULT_CONFIG = ROOT / "configs" / "retacred_qceasc_grouped_counterfactual_formal_single_seed.json"
...
ADAPTIVE_MEMORY_STATE_SCHEMA = "q-attention.q-ceasc-adaptive-memory.v2"
...
def _run(args: argparse.Namespace, pause: PauseController) -> int:
    ...
    run_dir = args.resume or args.output_dir or ROOT / "runs" / "retacred_qceasc_grouped_counterfactual_formal_single_seed" / f"{provisional_stamp}_seed{seed}"
...
def main() -> int:
    ...
    try:
        ...
        print(f"[q-ceasc] safely paused: {exc}", flush=True)
