#!/usr/bin/env python3
"""Run one complete Re-TACRED QK coherent-transport seed under a frozen contract."""
...
DEFAULT_CONFIG = ROOT / "configs" / "retacred_qk_coherent_transport_formal_single_seed.json"
RUN_MANIFEST_SCHEMA = "q-attention.qk-coherent-transport-batch-resume-run.v1"
DATA_MANIFEST_SCHEMA = "q-attention.qk-coherent-transport-materialized-data.v1"
...
ADAPTIVE_MEMORY_STATE_SCHEMA = "q-attention.qk-coherent-transport-adaptive-memory.v1"
BASELINE_ARTIFACTS = ("metrics.json", "model.pt", "vocab.json", "labels.json")
BASELINE_IMPORT_SCHEMA = "q-attention.qk-coherent-transport-legacy-baseline-import.v1"
...
def _source_contract() -> dict[str, Any]:
    paths = {
        "runner": ROOT / "experiments" / "run_retacred_qk_coherent_transport_formal_single_seed.py",
        "worker": ROOT / "experiments" / "run_qk_coherent_transport_selector_worker.py",
        ...
        / "adapters"
        / "attention_scores.py",
        "qk_coherent_transport": ROOT
        / "src"
        / "q_attention"
        / "plugins"
        / "q_query_key_coherent_transport.py",
    }
    return {
...
def selector_resume_contract(
    *,
    ...
    return {
        "stage": "qk_coherent_transport_selector",
        "training_semantics": {
            "selector": selector,
            "seed": int(seed),
...
def _render_selector_dashboard(
    ...
    lines = [
        (
            f"QK coherent-transport selectors: {counts['complete']}/{total} complete | "
    ...
    try:
        while pending or active:
            ...
            while pending and available and not (pause is not None and pause.requested):
                ...
                command = [
                    args.python_bin,
                    str(ROOT / "experiments" / "run_qk_coherent_transport_selector_worker.py"),
...
def build_kernel(
    ...
    *,
    ...
    mode_map = {
        "qk_coherent_transport": "quantum",
        "classical_coherent_transport": "classical",
    }
    if mode not in mode_map:
        raise ValueError(f"unknown QK coherent-transport selector {mode!r}")
    config_obj = QueryKeyCoherentTransportConfig(
        num_layers=model.config.num_layers,
        num_heads=model.config.num_heads,
        head_dim=model.config.dim // model.config.num_heads,
        register_qubits=int(kernel_config["register_qubits"]),
        depth=int(kernel_config["depth"]),
        angle_scale=float(kernel_config["angle_scale"]),
        max_phase=float(kernel_config["max_phase"]),
        max_post_rotation=float(kernel_config["max_post_rotation"]),
        max_transport=float(kernel_config["max_transport"]),
        initial_phase=float(kernel_config["initial_phase"]),
        initial_post_rotation=float(kernel_config["initial_post_rotation"]),
        initial_transport=float(kernel_config["initial_transport"]),
        pair_chunk_size=(
            int(kernel_config.get("pair_chunk_size", 4096))
    ...
    # Pair chunking is the only QK memory lever.  The transport kernel is a
    # pure score residual, so activation checkpointing/model sharding do not
    # alter its mathematical contract; those flags remain recorded by the
    # runner for a uniform execution and resume interface.
    del activation_checkpointing, model_parallel_devices
    return build_query_key_coherent_transport_kernel(mode_map[mode], config_obj)
...
def evaluate_selector(
    ...
    device: torch.device,
    label_count: int,
    kernel: QueryKeyCoherentTransportKernel | None,
    stage: str,
) -> dict[str, Any]:
    return evaluate(
        model,
...
def _run(args: argparse.Namespace, pause: PauseController) -> int:
    ...
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if config.get("schema_version") != "q-attention.qk-coherent-transport-formal-single-seed.v1":
        raise ValueError("unsupported QK coherent-transport formal config")
    ...
    if model_parallel_gpu_ids:
        raise ValueError(
            "QK coherent-transport formal handoff supports serial or selector-parallel GPUs only; "
    ...
    run_dir = args.resume or args.output_dir or ROOT / "runs" / "retacred_qk_coherent_transport_formal_single_seed" / f"{provisional_stamp}_seed13"
    ...
    if args.import_baseline_from is not None:
        ...
        print(
            "[qk-coherent-transport] imported completed legacy baseline; "
            "QK candidate and matched control will restart from batch 0",
            flush=True,
        )
    ...
    summary = {
        "schema_version": "q-attention.qk-coherent-transport-formal-single-seed.run.v1",
        "name": config["name"],
        "formal_experiment": True,
...
def main() -> int:
    ...
    try:
        ...
        print(f"[qk-coherent-transport] safely paused: {exc}", flush=True)
