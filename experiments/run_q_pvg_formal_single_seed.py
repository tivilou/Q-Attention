#!/usr/bin/env python3
"""Run the frozen Q-PVG Re-TACRED formal single-seed contract."""
...
import run_q_pvg_scheduler_base as _base
from run_q_pvg_transfer_base import (
    evaluate,
    build_kernel as _build_pvg_kernel,
    metric_delta,
    train_kernel,
)
...
SELECTOR_WORKER_PATH = ROOT / "experiments" / "run_q_pvg_selector_worker.py"
_base.DEFAULT_CONFIG = DEFAULT_CONFIG
_base.FORMAL_RUN_NAME = "retacred_q_pvg_formal_single_seed"
_base.SELECTOR_WORKER_PATH = SELECTOR_WORKER_PATH
_base.FORMAL_RUN_NAME = "retacred_q_pvg_formal_single_seed"
_base.FORMAL_CONFIG_SCHEMAS = {
    "q-attention.q-pvg-formal-single-seed.v1",
}
...
def _pvg_source_contract(*, counterfactual: bool = False) -> dict[str, Any]:
    del counterfactual
    paths = {
        "runner": ROOT / "experiments" / "run_q_pvg_formal_single_seed.py",
        "scheduler": ROOT / "experiments" / "run_q_pvg_scheduler_base.py",
        "worker": SELECTOR_WORKER_PATH,
        "baseline_trainer": ROOT / "experiments" / "train_relation_baseline.py",
        "kernel_trainer": ROOT / "experiments" / "run_q_pvg_transfer_base.py",
        ...
        "attention_adapter": ROOT / "src" / "q_attention" / "adapters" / "attention_context.py",
        "attention_context_export": ROOT / "src" / "q_attention" / "adapters" / "__init__.py",
        "q_pvg": ROOT / "src" / "q_attention" / "plugins" / "q_pvg.py",
    }
...
def selector_resume_contract(**kwargs: Any) -> dict[str, Any]:
    ...
    contract["stage"] = "q_pvg_selector"
    contract["source"] = _pvg_source_contract()
    return contract
...
_base._source_contract = _pvg_source_contract
...
def build_kernel(
    ...
    *,
    ...
    built = _build_pvg_kernel(mode, model, seed, args, config_override=kernel)
    # Query chunking is an execution control owned by the context adapter, not
    # by QPVGConfig (and therefore must not enter the scientific kernel config).
...
def _rewrite_summary(run_dir: Path, config: dict[str, Any]) -> None:
    ...
    payload["schema_version"] = "q-attention.q-pvg-formal-single-seed.run.v1"
