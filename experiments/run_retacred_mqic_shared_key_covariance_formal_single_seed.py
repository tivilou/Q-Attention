#!/usr/bin/env python3
"""Run one complete Re-TACRED MQIC shared-key covariance seed under a frozen contract."""
...
DEFAULT_CONFIG = ROOT / "configs" / "retacred_mqic_shared_key_covariance_formal_single_seed.json"
RUN_MANIFEST_SCHEMA = "q-attention.mqic-shared-key-covariance-batch-resume-run.v1"
DATA_MANIFEST_SCHEMA = "q-attention.mqic-shared-key-covariance-materialized-data.v1"
...
ADAPTIVE_MEMORY_STATE_SCHEMA = "q-attention.mqic-shared-key-covariance-adaptive-memory.v1"
BASELINE_ARTIFACTS = ("metrics.json", "model.pt", "vocab.json", "labels.json")
BASELINE_IMPORT_SCHEMA = "q-attention.mqic-shared-key-covariance-legacy-baseline-import.v1"
...
def _source_contract() -> dict[str, Any]:
    paths = {
        "runner": ROOT / "experiments" / "run_retacred_mqic_shared_key_covariance_formal_single_seed.py",
        "worker": ROOT / "experiments" / "run_mqic_shared_key_covariance_selector_worker.py",
        ...
        / "adapters"
        / "attention_scores.py",
        "mqic_shared_key_covariance": ROOT
        / "src"
        / "q_attention"
        / "plugins"
        / "q_multi_query_covariance.py",
    }
    return {
...
def selector_resume_contract(
    *,
    ...
    return {
        "stage": "mqic_shared_key_covariance_selector",
        "training_semantics": {
            "selector": selector,
            "seed": int(seed),
...
def _render_selector_dashboard(
    ...
    lines = [
        (
            f"MQIC shared-key covariance selectors: {counts['complete']}/{total} complete | "
    ...
    try:
        while pending or active:
            ...
            while pending and available and not (pause is not None and pause.requested):
                ...
                command = [
                    args.python_bin,
                    str(ROOT / "experiments" / "run_mqic_shared_key_covariance_selector_worker.py"),
...
def build_kernel(
    ...
    *,
    ...
    mode_map = {
        "mqic_shared_key_covariance": "quantum",
        "classical_mqic_shared_key_covariance": "classical",
    }
    if mode not in mode_map:
        raise ValueError(f"unknown MQIC shared-key covariance selector {mode!r}")
    config_obj = MultiQueryCovarianceConfig(
        ...
        max_covariance=float(kernel_config["max_covariance"]),
    ...
    # Pair chunking is the MQIC memory lever. Activation checkpointing and
    # model sharding do not alter this pure score-residual contract; the flags
    # remain recorded by the runner for a uniform execution/resume interface.
    del activation_checkpointing, model_parallel_devices
    return build_multi_query_covariance_kernel(mode_map[mode], config_obj)
...
def evaluate_selector(
    ...
    device: torch.device,
    label_count: int,
    kernel: MultiQueryCovarianceKernel | None,
    stage: str,
) -> dict[str, Any]:
    return evaluate(
        model,
...
def _run(args: argparse.Namespace, pause: PauseController) -> int:
    ...
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if config.get("schema_version") != "q-attention.mqic-shared-key-covariance-formal-single-seed.v1":
        raise ValueError("unsupported MQIC shared-key covariance formal config")
    ...
    if model_parallel_gpu_ids:
        raise ValueError(
            "MQIC shared-key covariance formal handoff supports serial or selector-parallel GPUs only; "
    ...
    run_dir = args.resume or args.output_dir or ROOT / "runs" / "retacred_mqic_shared_key_covariance_formal_single_seed" / f"{provisional_stamp}_seed13"
    ...
    if args.import_baseline_from is not None:
        ...
        print(
            "[mqic-shared-key-covariance] imported completed legacy baseline; "
            "MQIC candidate and matched control will restart from batch 0",
            flush=True,
        )
    ...
    summary = {
        "schema_version": "q-attention.mqic-shared-key-covariance-formal-single-seed.run.v1",
        "name": config["name"],
        "formal_experiment": True,
...
def main() -> int:
    ...
    try:
        ...
        print(f"[mqic-shared-key-covariance] safely paused: {exc}", flush=True)
