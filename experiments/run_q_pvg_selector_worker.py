#!/usr/bin/env python3
"""Run one Q-PVG selector worker for the formal scheduler."""
...
from run_q_pvg_transfer_base import (  # noqa: E402
    context_hook_config,
    train_kernel,
)
from run_q_pvg_formal_single_seed import (  # noqa: E402
    CUDA_OOM_EXIT_CODE,
...
def write_case_study(
    *,
    ...
    def _forward_capture(
        ...
        def _register_common(mode: str) -> list[torch.utils.hooks.RemovableHandle]:
            ...
            for layer_index in range(int(model.config.num_layers)):
                ...
                context_path = f"encoder.layers.{layer_index}.attn.context_intervention"
                context_module = resolve_module(model, context_path)

                def context_hook(_m, inputs, output, index=layer_index, capture_mode=mode):
                    if len(inputs) < 4 or not all(isinstance(item, torch.Tensor) for item in inputs[:4]):
                        return
                    captures[f"{capture_mode}_scores"][index] = {
                        "input": inputs[3].detach(),
                        "output": output.detach() if isinstance(output, torch.Tensor) else None,
                        "value": inputs[2].detach(),
                    }

                handles.append(context_module.register_forward_hook(context_hook))
            return handles
        ...
        with torch.no_grad():
            ...
            adapter = AttentionContextKernelAdapter(
                ...
                kernel,
                query_chunk_size=getattr(kernel, "query_chunk_size", None),
            )
            adapter.attach(context_hook_config(batch, capture_trace=True))
            ...
            try:
                ...
                captures["pvg_trace"] = {
                    ...
                    for index, trace in adapter.last_traces.items()
                }
    ...
    for checkpoint_label, state, checkpoint_slug in checkpoint_states:
        ...
        for split in ("train", "valid", "test"):
            ...
            for local_index, record_index in enumerate(_indices_for(split)):
                ...
                trace_by_layer = captures.get("pvg_trace", {})
                for rep_id, trace_key, axes in (
                    ("q_pvg_query_state_real", "query_state_real", ["layers", "heads", "query_tokens", "state"]),
                    ("q_pvg_query_state_imag", "query_state_imag", ["layers", "heads", "query_tokens", "state"]),
                    ("q_pvg_key_state_real", "key_state_real", ["layers", "heads", "key_tokens", "state"]),
                    ("q_pvg_key_state_imag", "key_state_imag", ["layers", "heads", "key_tokens", "state"]),
                    ("q_pvg_alignment_real", "alignment_real", ["layers", "heads", "query_tokens", "key_tokens"]),
                    ("q_pvg_alignment_imag", "alignment_imag", ["layers", "heads", "query_tokens", "key_tokens"]),
                    ("q_pvg_gate", "gate", ["layers", "heads", "query_tokens", "key_tokens"]),
                    ("q_pvg_score_adjustment", "score_adjustment", ["layers", "heads", "query_tokens", "key_tokens"]),
                    ("q_pvg_attention", "attention", ["layers", "heads", "query_tokens", "key_tokens"]),
                    ("q_pvg_routed_values", "routed_values", ["layers", "heads", "query_tokens", "key_tokens", "value_dim"]),
                    ("q_pvg_output", "output", ["layers", "heads", "query_tokens", "value_dim"]),
                ):
                    rows = []
                    for layer_index in range(int(model.config.num_layers)):
                ...
                stages_by_case.append({
                    ...
                    "stages": [
                        ...
                        {"stage": "scoring", "status": "observed", "observed_fields": {"representations": ["attention_qkv", "baseline_attention_scores", "q_pvg_alignment_real", "q_pvg_alignment_imag", "q_pvg_gate"]}},
                        {"stage": "selection", "status": "observed", "observed_fields": {"representations": ["q_pvg_score_adjustment", "steered_attention_scores"]}},
                        {"stage": "context", "status": "observed", "observed_fields": {"representations": ["q_pvg_routed_values", "q_pvg_output"], "selector_prediction": selector_prediction}},
    ...
    payload = {
        "schema_version": "q-attention.q-pvg-case-study.v1",
        "selector": selector,
        "status": "observed",
        ...
        "provenance": {
            ...
            "plugin_sha256": _sha256(ROOT / "src" / "q_attention" / "plugins" / "q_pvg.py"),
    ...
    sample_trace = {
        ...
        "experiment": {"run_id": output_dir.parent.parent.name, "dataset": "retacred.train+valid+test", "code_revision": _git_revision(), "config_sha256": _sha256(config_path), "model_identity": "relation-transformer-q-pvg", "seed": int(config["seed"])},
