#!/usr/bin/env python3
"""Shared scheduler for the Q-EPVG formal single-seed handoff."""
...
SELECTOR_WORKER_PATH = EXPERIMENTS / "run_q_epvg_selector_worker.py"
...
try:
    ...
    def format_gpu_memory(value: Any) -> str:
        ...
        return "gpu-memory " + ", ".join(f"{key}={item}" for key, item in value.items())
...
FORMAL_RUN_NAME = "retacred_q_epvg_formal_single_seed"
REPLICATION_SEEDS = (13, 29, 53)
RUN_MANIFEST_SCHEMA = "q-attention.q-epvg-batch-resume-run.v1"
DATA_MANIFEST_SCHEMA = "q-attention.q-epvg-materialized-data.v1"
...
# Adaptive execution is ordered by throughput first: start at logical/physical
# batch 256 with no query chunking or activation checkpointing.  After an OOM,
# reduce the physical micro-batch while retaining logical batch 256 through
# accumulation.  Only after the 32-sample tier still OOMs do we introduce query
# chunking, beginning at all queries and then halving it by powers of two.
...
ADAPTIVE_HARDWARE_PROFILES: tuple[dict[str, Any], ...] = tuple(
    ...
    + [
        {
            "name": f"adaptive_chunk_{divisor}x_micro_32",
            "pair_chunk_size": None,
        ...
        for divisor in _CHUNK_DIVISORS[1:]
    ]
)
CUDA_OOM_MARKERS = (
    "cuda out of memory",
...
ADAPTIVE_MEMORY_STATE_SCHEMA = "q-attention.q-epvg-adaptive-memory.v2"
BASELINE_ARTIFACTS = ("metrics.json", "model.pt", "vocab.json", "labels.json")
BASELINE_IMPORT_SCHEMA = "q-attention.q-epvg-legacy-baseline-import.v1"
ELASTIC_RESUME_SOURCE_FILES = (
    "runner",
    # Older Q-EPVG manifests named the orchestration module separately.
    "scheduler",
    "worker",
    "baseline_trainer",
    "kernel_trainer",
...
FORMAL_CONFIG_SCHEMAS = {"q-attention.q-epvg-formal-single-seed.v1"}
...
def _source_contract(*, counterfactual: bool = False) -> dict[str, Any]:
    ...
    if counterfactual:
        paths["q_epvg_context"] = (
            ROOT / "src" / "q_attention" / "plugins" / "q_epvg.py"
        )
    return {
    ...
    if requested == "adaptive":
        ...
        return {
            **tiers[0],
            ...
            "tiers": tiers,
            "selection_reason": (
                "start at logical/physical batch 256 with no query chunking; after "
                "OOM use 128/64/32 physical micro-batches with accumulation, then "
                "halve the all-pairs query chunk from all when the 32-sample tier "
                "still cannot fit"
            ),
        }
    if requested == "config":
...
def _validate_legacy_run_metadata(
    ...
    """Validate optional metadata emitted by a legacy formal runner."""
    ...
    if run_config_path.is_file():
        ...
        for key, expected in scalar_contract.items():
            # A completed Q-TRIAD baseline is intentionally importable.  The
            # baseline/model/data contract is checked below; selector names
            # and experiment schema belong to the old runner and are not
            # evidence that the baseline tensors are incompatible.
            if key in {"schema_version", "name", "selectors", "candidate", "matched_control"}:
                continue
    ...
    if summary_path.is_file():
        ...
        for key, expected in (("formal_experiment", True), ("seed", seed)):
    ...
    *,
    ...
    if legacy_data_manifest.is_file():
        ...
        if manifest.get("schema_version") not in {
            ...
            "q-attention.qtriad-materialized-data.v1",
...
def _render_selector_dashboard(
    ...
    lines = [
        (
            f"Q-EPVG selectors: {counts['complete']}/{total} complete | "
...
def _run(args: argparse.Namespace, pause: PauseController) -> int:
    ...
    if config.get("schema_version") not in FORMAL_CONFIG_SCHEMAS:
        raise ValueError("unsupported Q-EPVG formal config")
    ...
    if model_parallel_gpu_ids and config.get("counterfactual"):
        raise ValueError(
            "Q-EPVG formal handoff supports selector-parallel GPUs only; "
            "model-parallel Case Study capture is intentionally unsupported"
        )
    if model_parallel_gpu_ids:
        raise ValueError(
            "Q-EPVG formal handoff supports serial or selector-parallel GPUs only; "
    ...
    run_dir = args.resume or args.output_dir or ROOT / "runs" / FORMAL_RUN_NAME / f"{provisional_stamp}_seed{seed}"
    ...
    if args.import_baseline_from is not None:
        ...
        print(
            "[q-epvg] imported completed legacy baseline; "
            "Q-EPVG candidate and matched control will restart from batch 0",
            flush=True,
        )
    ...
    if args.baseline_only:
        _write_json_atomic(
            ...
            {
                "schema_version": "q-attention.q-epvg-baseline-stage.v1",
                "stage": "baseline",
                "seed": seed,
                "valid": baseline_valid,
                "test": baseline_test,
        ...
        print("[q-epvg] baseline stage complete; selector tasks are deferred", flush=True)
        return 0
    ...
    structural = by_name[config["structural_control"]]
    ...
    candidate_minus_structural = metric_delta(candidate["test"]["metrics"], structural["test"]["metrics"])
    ...
    gates = {
        ...
        "candidate_minus_random_group_macro_f1": candidate_minus_structural["delta_macro_f1"],
        ...
        "random_group_control_completed": bool(structural.get("finite")),
        "finite_metrics": all(row["finite"] for row in rows),
        "test_used_for_training_or_selection": False,
    }
    summary = {
        "schema_version": (
            "q-attention.q-epvg-formal-single-seed.run.v1"
        ),
        "name": config["name"],
        "formal_experiment": True,
        ...
        "candidate": config["candidate"],
        "matched_control": config["matched_control"],
        "structural_control": config["structural_control"],
        ...
        "candidate_minus_random_group": candidate_minus_structural,
        "relative_deltas": {
    ...
    # Keep a compact, machine-readable identity artifact beside the human
    # summary.  Exporters require this file so a completed run cannot be
    # mistaken for an older report with incomplete provenance.
    summary_data = {
    ...
    lines = [
        ...
        f"- structural control: `{config['structural_control']}`",
        ...
        *([
        ...
        f"- candidate minus random-group test macro-F1: `{candidate_minus_structural['delta_macro_f1']:.6f}`",
        ...
        f"- random-group control completed: `{str(gates['random_group_control_completed']).lower()}`",
        "",
        (
...
def main() -> int:
    ...
    try:
        ...
        print(f"[q-epvg] safely paused: {exc}", flush=True)
