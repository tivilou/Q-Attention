#!/usr/bin/env python3
"""Read-only diagnosis for a Q-EPVG batch-resume directory.
...
RUNNER_NAME = "run_q_epvg_formal_single_seed.py"
DEFAULT_CONFIG_NAME = "retacred_q_epvg_formal_single_seed.json"
...
def _load_runner(repo_root: Path) -> tuple[Any | None, str | None]:
    ...
    if not runner_path.is_file():
        return None, "current Q-EPVG runner is missing"
    for path in (repo_root / "src", repo_root / "experiments"):
        if str(path) not in sys.path:
            sys.path.insert(0, str(path))
    spec = importlib.util.spec_from_file_location("q_epvg_resume_runner", runner_path)
    if spec is None or spec.loader is None:
        return None, "current Q-EPVG runner cannot be loaded"
    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
    except Exception as exc:  # environment-specific imports can fail on the collaborator host
        return None, f"current Q-EPVG runner import failed ({type(exc).__name__})"
    return module, None
...
def _build_current_contract(
    ...
    *,
    ...
    if config is None:
        return None, {}, f"Q-EPVG config is {config_error}"
...
def _emit(result: dict[str, Any], json_only: bool) -> int:
    if json_only:
        ...
        print("Q-EPVG resume diagnostic (read only)")
...
def main() -> int:
    ...
    if runner is None:
        result["errors"].append(runner_error or "Q-EPVG runner could not be loaded")
        return _emit(result, args.json)
