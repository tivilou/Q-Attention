...
for selector in q_ceasc_grouped_counterfactual classical_grouped_counterfactual random_grouped_counterfactual; do
  ...
  CONFIG_SHA256=$(sha256sum configs/retacred_qceasc_grouped_counterfactual_formal_single_seed.json | awk '{print $1}')
  ...
  "${PYTHON_BIN}" scripts/validate_qceasc_grouped_counterfactual_case_study.py "${RUN_DIR}/selectors/${selector}" --config-sha256 "${CONFIG_SHA256}"
done
...
assert p.get("candidate") == "q_ceasc_grouped_counterfactual"
assert p.get("matched_control") == "classical_grouped_counterfactual"
assert p.get("structural_control") == "random_grouped_counterfactual"
assert p.get("schema_version") == "q-attention.q-ceasc-grouped-counterfactual-formal-single-seed.run.v1"
...
data = p.get("data")
assert isinstance(data, dict), "run summary must include data provenance"
assert set(data) == {"train", "valid", "test"}, "run summary data must cover train/valid/test"
for split in ("train", "valid", "test"):
    info = data[split]
    assert isinstance(info, dict) and info, f"run summary data.{split} must be non-empty"
    assert int(info.get("records", 0)) > 0, f"run summary data.{split}.records must be positive"
    assert info.get("source_sha256"), f"run summary data.{split} must include source_sha256"
gates = p.get("gates")
assert isinstance(gates, dict) and "l1_utility_gate" in gates and "quantum_inspired_relative_gain_gate" in gates and gates.get("random_group_control_completed") is True, "run summary must include separated rating gates and completed random-group control"
PY

DEFAULT_REPORT_DIR="reports/retacred_qceasc_grouped_counterfactual_formal_single_seed/${RUN_NAME}"
REPORT_DIR=${REPORT_DIR:-${DEFAULT_REPORT_DIR}}
[[ "${REPORT_DIR}" = /* ]] || REPORT_DIR="${ROOT}/${REPORT_DIR}"
REPORT_DIR=$(readlink -m "${REPORT_DIR}")
case "${REPORT_DIR}" in "${ROOT}/reports/retacred_qceasc_grouped_counterfactual_formal_single_seed/"*) ;; *) echo "Report must be under the Q-CEASC grouped counterfactual report root." >&2; exit 1;; esac
...
cp configs/retacred_qceasc_grouped_counterfactual_formal_single_seed.json "${REPORT_DIR}/run_config.json"
...
git commit -m "Add Q-CEASC grouped counterfactual formal single-seed report"
git push origin 1.1
echo "REPORT_DIR=${REPORT_REL}"
