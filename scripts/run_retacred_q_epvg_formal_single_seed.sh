...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h) exec "${PYTHON_BIN}" "${ROOT}/experiments/run_q_epvg_formal_single_seed.py" --help;;
...
RUN_DIR="${RESUME_DIR:-${OUTPUT_DIR:-runs/retacred_q_epvg_formal_single_seed/${STAMP}_seed13}}"
if [[ -z "${RESUME_DIR}" ]]; then
  "${PYTHON_BIN}" scripts/check_retacred_q_epvg_formal_single_seed.py --config configs/retacred_q_epvg_formal_single_seed.json --fresh-run
fi
COMMAND=("${PYTHON_BIN}" experiments/run_q_epvg_formal_single_seed.py --config configs/retacred_q_epvg_formal_single_seed.json --device cuda --gpus "${GPU_SPEC}" --hardware-profile "${HARDWARE_PROFILE}" --python-bin "${PYTHON_BIN}")
...
exec scripts/export_retacred_q_epvg_formal_single_seed_report.sh "${EXPORT_ARGS[@]}"
