...
DEFAULT_CONFIG_NAME = "retacred_qceasc_formal_single_seed.json"
...
def main() -> int:
    ...
    if args.fresh:
        ...
        elif config.get("schema_version") != "q-attention.q-ceasc-formal-single-seed.v1":
            errors.append("config has an unsupported Q-CEASC schema")
        elif config.get("selectors") != [
            "disabled", "q_ceasc", "classical_ceasc"
        ]:
            errors.append("config selectors must be disabled, Q-CEASC candidate, and matched control")
        if runner is None:
        ...
        if args.json:
            ...
            print("Q-CEASC fresh-run preflight (READ ONLY)")
...
def _emit(result: dict[str, Any], json_only: bool, *, failure: bool) -> int:
    if json_only:
        ...
        print("Q-CEASC resume diagnostic (READ ONLY)")
