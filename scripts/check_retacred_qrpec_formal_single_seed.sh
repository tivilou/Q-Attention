...
for file in \
    configs/retacred_qrpec_formal_single_seed.json \
    experiments/run_retacred_qrpec_formal_single_seed.py \
    experiments/run_qrpec_selector_worker.py \
    ...
    src/q_attention/plugins/q_relation_perturbation_echo_curvature.py \
    experiments/run_q_causal_value_evidence_relation_smoke.py \
    experiments/run_q_causal_value_evidence_relation_transfer.py \
    scripts/run_retacred_qrpec_formal_single_seed.sh \
    scripts/export_retacred_qrpec_formal_single_seed_report.sh \
    docs/current/retacred_qrpec_formal_single_seed_zh.md; do
...
p = json.loads(Path("configs/retacred_qrpec_formal_single_seed.json").read_text(encoding="utf-8"))
assert p["formal_experiment"] is True
assert p["seed"] == 13
assert p["candidate"] == "q_rpec"
assert p["matched_control"] == "classical_local_echo"
assert p["gates"]["test_used_for_training_or_selection"] is False
print("Q-RPEC formal config=OK")
PY

PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" "${PYTHON_BIN}" -m pytest -q \
  tests/test_q_relation_perturbation_echo_curvature.py
"${PYTHON_BIN}" -m py_compile experiments/run_retacred_qrpec_formal_single_seed.py experiments/run_qrpec_selector_worker.py src/q_attention/plugins/q_relation_perturbation_echo_curvature.py
nvidia-smi --query-gpu=index,name,memory.total --format=csv,noheader
echo "Q-RPEC formal preflight=OK"
git rev-parse HEAD
