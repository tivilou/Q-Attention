#!/usr/bin/env python3
"""Diagnose a MQIC shared-key covariance batch-resume directory without changing it.
...
RUNNER_NAME = "run_retacred_mqic_shared_key_covariance_formal_single_seed.py"
DEFAULT_CONFIG_NAME = "retacred_mqic_shared_key_covariance_formal_single_seed.json"
...
def _load_runner(repo_root: Path) -> tuple[Any | None, str | None]:
    ...
    spec = importlib.util.spec_from_file_location("mqic_shared_key_covariance_resume_runner", runner_path)
...
def _parse_args() -> argparse.Namespace:
    ...
    target.add_argument(
        "--fresh",
        action="store_true",
        help="validate the checked-out MQIC contract before creating a new run",
    )
...
def main() -> int:
    ...
    if args.fresh:
        ...
        elif config.get("schema_version") != "q-attention.mqic-shared-key-covariance-formal-single-seed.v1":
            errors.append("config has an unsupported MQIC shared-key covariance schema")
        elif config.get("selectors") != [
            "disabled", "mqic_shared_key_covariance", "classical_mqic_shared_key_covariance"
        ]:
            errors.append("config selectors must be disabled, MQIC candidate, and matched control")
        if runner is None:
        ...
        if args.json:
            ...
            print("MQIC shared-key covariance fresh-run preflight (READ ONLY)")
...
def _emit(result: dict[str, Any], json_only: bool, *, failure: bool) -> int:
    if json_only:
        ...
        print("MQIC shared-key covariance resume diagnostic (READ ONLY)")
