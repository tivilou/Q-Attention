...
usage() {
  echo "Usage: bash scripts/check_qvres_relation_transfer_full.sh [--skip-tests] [--allow-dirty]"
}
...
if [[ ${ALLOW_DIRTY} -eq 0 ]]; then
  ...
  if [[ -n "${GIT_STATUS}" ]]; then
    echo "Repository is dirty. Commit or restore local changes before the formal run:" >&2
    printf '%s\n' "${GIT_STATUS}" >&2
    exit 1
  fi
fi
...
"${PYTHON_BIN}" -c "import json; p=json.load(open('configs/q_vres_relation_transfer_full.json')); assert p['formal_experiment'] is True; assert all(p[k] == 0 for k in ('max_train_records','max_valid_records','max_test_records')); print('formal config=OK')"
...
PYTHONPATH="${ROOT}/src${PYTHONPATH:+:${PYTHONPATH}}" "${PYTHON_BIN}" -c "from q_attention.plugins.q_causal_value_evidence import build_causal_value_transport_kernel; print('Q-VRES import=OK')"
