...
for selector in qk_coherent_transport classical_coherent_transport; do
...
assert p.get("candidate") == "qk_coherent_transport"
assert p.get("matched_control") == "classical_coherent_transport"
...
DEFAULT_REPORT_DIR="reports/retacred_qk_coherent_transport_formal_single_seed/${RUN_NAME}"
REPORT_DIR=${REPORT_DIR:-${DEFAULT_REPORT_DIR}}
[[ "${REPORT_DIR}" = /* ]] || REPORT_DIR="${ROOT}/${REPORT_DIR}"
REPORT_DIR=$(readlink -m "${REPORT_DIR}")
case "${REPORT_DIR}" in "${ROOT}/reports/retacred_qk_coherent_transport_formal_single_seed/"*) ;; *) echo "Report must be under the QK coherent-transport report root." >&2; exit 1;; esac
...
cp configs/retacred_qk_coherent_transport_formal_single_seed.json "${REPORT_DIR}/run_config.json"
