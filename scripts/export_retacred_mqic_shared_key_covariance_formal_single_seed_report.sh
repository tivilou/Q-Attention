...
for selector in mqic_shared_key_covariance classical_mqic_shared_key_covariance; do
...
assert p.get("candidate") == "mqic_shared_key_covariance"
assert p.get("matched_control") == "classical_mqic_shared_key_covariance"
...
DEFAULT_REPORT_DIR="reports/retacred_mqic_shared_key_covariance_formal_single_seed/${RUN_NAME}"
REPORT_DIR=${REPORT_DIR:-${DEFAULT_REPORT_DIR}}
[[ "${REPORT_DIR}" = /* ]] || REPORT_DIR="${ROOT}/${REPORT_DIR}"
REPORT_DIR=$(readlink -m "${REPORT_DIR}")
case "${REPORT_DIR}" in "${ROOT}/reports/retacred_mqic_shared_key_covariance_formal_single_seed/"*) ;; *) echo "Report must be under the MQIC shared-key covariance report root." >&2; exit 1;; esac
...
cp configs/retacred_mqic_shared_key_covariance_formal_single_seed.json "${REPORT_DIR}/run_config.json"
