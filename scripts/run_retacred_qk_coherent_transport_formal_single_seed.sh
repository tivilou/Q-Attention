...
usage() { echo "Usage: bash scripts/run_retacred_qk_coherent_transport_formal_single_seed.sh [--gpu N[,N...]|auto] [--hardware-profile config|auto|adaptive|low_memory|balanced|high_memory] [--output-dir PATH | --resume RUN_DIR | --import-baseline-from OLD_RUN_DIR] [--allow-gpu-topology-change] [--allow-code-update] [--report-dir PATH] [--log-every-batches N] [--checkpoint-every-batches N] [--skip-preflight] [--dry-run]"; }
while [[ $# -gt 0 ]]; do
  case "$1" in
...
RUN_DIR=${RESUME_DIR:-${OUTPUT_DIR:-runs/retacred_qk_coherent_transport_formal_single_seed/${STAMP}_seed13}}
...
COMMAND=(
  "${PYTHON_BIN}" experiments/run_retacred_qk_coherent_transport_formal_single_seed.py
  --config configs/retacred_qk_coherent_transport_formal_single_seed.json
  --device cuda
  --seed 13
...
EXPORT_COMMAND=(bash scripts/export_retacred_qk_coherent_transport_formal_single_seed_report.sh --run-dir "${RUN_DIR}")
