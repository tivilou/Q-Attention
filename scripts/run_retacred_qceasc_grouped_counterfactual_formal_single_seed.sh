...
CONFIG_PATH=configs/retacred_qceasc_grouped_counterfactual_formal_single_seed.json
EXPERIMENT_NAME=retacred_qceasc_grouped_counterfactual_formal_single_seed
CHECK_SCRIPT=scripts/check_retacred_qceasc_grouped_counterfactual_formal_single_seed.py
EXPORT_SCRIPT=scripts/export_retacred_qceasc_grouped_counterfactual_formal_single_seed_report.sh
GPU_SPEC=auto
OUTPUT_DIR=
RESUME_DIR=
...
usage() { echo "Usage: bash scripts/run_retacred_qceasc_grouped_counterfactual_formal_single_seed.sh [--gpu N[,N...]|auto] [--hardware-profile config|auto|adaptive|low_memory|balanced|high_memory] [--output-dir PATH | --resume RUN_DIR | --import-baseline-from OLD_RUN_DIR] [--allow-gpu-topology-change] [--allow-code-update] [--report-dir PATH] [--log-every-batches N] [--checkpoint-every-batches N] [--skip-preflight] [--dry-run]"; }
while [[ $# -gt 0 ]]; do
  case "$1" in
...
RUN_DIR=${RESUME_DIR:-${OUTPUT_DIR:-runs/${EXPERIMENT_NAME}/${STAMP}_seed13}}
...
if [[ ${SKIP_PREFLIGHT} -eq 0 && ${DRY_RUN} -eq 0 ]]; then
  if [[ -n "${RESUME_DIR}" ]]; then
    "${PYTHON_BIN}" "${CHECK_SCRIPT}" \
      --run-dir "${RUN_DIR}" --gpus "${GPU_SPEC}" --device cuda \
      --hardware-profile "${HARDWARE_PROFILE}"
  else
    "${PYTHON_BIN}" "${CHECK_SCRIPT}" --fresh
  fi
fi

COMMAND=(
  "${PYTHON_BIN}" experiments/run_retacred_qceasc_grouped_counterfactual_formal_single_seed.py
  --config "${CONFIG_PATH}"
  --device cuda
  --seed 13
...
EXPORT_COMMAND=(bash "${EXPORT_SCRIPT}" --run-dir "${RUN_DIR}")
