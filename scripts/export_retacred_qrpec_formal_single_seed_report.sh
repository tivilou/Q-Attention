...
for selector in q_rpec classical_local_echo; do
...
assert p.get("candidate") == "q_rpec"
assert p.get("matched_control") == "classical_local_echo"
...
DEFAULT_REPORT_DIR="reports/retacred_qrpec_formal_single_seed/${RUN_NAME}"
REPORT_DIR=${REPORT_DIR:-${DEFAULT_REPORT_DIR}}
[[ "${REPORT_DIR}" = /* ]] || REPORT_DIR="${ROOT}/${REPORT_DIR}"
REPORT_DIR=$(readlink -m "${REPORT_DIR}")
case "${REPORT_DIR}" in "${ROOT}/reports/retacred_qrpec_formal_single_seed/"*) ;; *) echo "Report must be under the Q-RPEC report root." >&2; exit 1;; esac
...
cp configs/retacred_qrpec_formal_single_seed.json "${REPORT_DIR}/run_config.json"
...
git commit -m "Add Q-RPEC Re-TACRED formal single-seed report"
git push origin 1.1
echo "REPORT_DIR=${REPORT_REL}"
