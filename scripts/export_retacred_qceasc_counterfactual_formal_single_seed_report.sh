...
for selector in q_ceasc_counterfactual classical_counterfactual; do
  ...
  CONFIG_SHA256=$(sha256sum configs/retacred_qceasc_counterfactual_formal_single_seed.json | awk '{print $1}')
  ...
  "${PYTHON_BIN}" scripts/validate_qceasc_counterfactual_case_study.py "${RUN_DIR}/selectors/${selector}" --config-sha256 "${CONFIG_SHA256}"
done
...
assert p.get("candidate") == "q_ceasc_counterfactual"
assert p.get("matched_control") == "classical_counterfactual"
assert p.get("schema_version") == "q-attention.q-ceasc-counterfactual-formal-single-seed.run.v1"
...
DEFAULT_REPORT_DIR="reports/retacred_qceasc_counterfactual_formal_single_seed/${RUN_NAME}"
REPORT_DIR=${REPORT_DIR:-${DEFAULT_REPORT_DIR}}
[[ "${REPORT_DIR}" = /* ]] || REPORT_DIR="${ROOT}/${REPORT_DIR}"
REPORT_DIR=$(readlink -m "${REPORT_DIR}")
case "${REPORT_DIR}" in "${ROOT}/reports/retacred_qceasc_counterfactual_formal_single_seed/"*) ;; *) echo "Report must be under the Q-CEASC counterfactual report root." >&2; exit 1;; esac
...
cp configs/retacred_qceasc_counterfactual_formal_single_seed.json "${REPORT_DIR}/run_config.json"
