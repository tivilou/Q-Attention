#!/usr/bin/env python3
"""Diagnose a QK coherent-transport batch-resume directory without changing it.
...
RUNNER_NAME = "run_retacred_qk_coherent_transport_formal_single_seed.py"
DEFAULT_CONFIG_NAME = "retacred_qk_coherent_transport_formal_single_seed.json"
...
def _load_runner(repo_root: Path) -> tuple[Any | None, str | None]:
    ...
    spec = importlib.util.spec_from_file_location("qk_coherent_transport_resume_runner", runner_path)
...
def _parse_args() -> argparse.Namespace:
    ...
    target.add_argument(
        "--fresh",
        action="store_true",
        help="validate the checked-out QK contract before creating a new run",
    )
...
def main() -> int:
    ...
    if args.fresh:
        ...
        elif config.get("schema_version") != "q-attention.qk-coherent-transport-formal-single-seed.v1":
            errors.append("config has an unsupported QK coherent-transport schema")
        elif config.get("selectors") != [
            "disabled", "qk_coherent_transport", "classical_coherent_transport"
        ]:
            errors.append("config selectors must be disabled, QK candidate, and matched control")
        if runner is None:
        ...
        if args.json:
            ...
            print("QK coherent-transport fresh-run preflight (READ ONLY)")
...
def _emit(result: dict[str, Any], json_only: bool, *, failure: bool) -> int:
    if json_only:
        ...
        print("QK coherent-transport resume diagnostic (READ ONLY)")
