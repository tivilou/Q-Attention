...
assert p.get("candidate") == "q_ceasc"
assert p.get("matched_control") == "classical_ceasc"
...
case "${REPORT_DIR}" in "${ROOT}/reports/retacred_qceasc_formal_single_seed/"*) ;; *) echo "Report must be under the Q-CEASC report root." >&2; exit 1;; esac
...
cp configs/retacred_qceasc_formal_single_seed.json "${REPORT_DIR}/run_config.json"
