#!/usr/bin/env python3
"""Diagnose a Q-CEASC batch-resume directory without changing it.
...
RUNNER_NAME = "run_retacred_qceasc_grouped_counterfactual_formal_single_seed.py"
DEFAULT_CONFIG_NAME = "retacred_qceasc_grouped_counterfactual_formal_single_seed.json"
SUPPORTED_SCHEMA = "q-attention.q-ceasc-grouped-counterfactual-formal-single-seed.v1"
EXPECTED_SELECTORS = ["disabled", "q_ceasc_grouped_counterfactual", "classical_grouped_counterfactual", "random_grouped_counterfactual"]
...
def _load_runner(repo_root: Path) -> tuple[Any | None, str | None]:
    ...
    spec = importlib.util.spec_from_file_location("q_ceasc_resume_runner", runner_path)
...
def _case_study_errors(selector_dir: Path, config_sha256: str) -> list[str]:
    errors: list[str] = []
    case_path = selector_dir / "case_study.json"
    trace_path = selector_dir / "sample_trace.json"
    case, case_error = _read_json(case_path)
    trace, trace_error = _read_json(trace_path)
    if case is None:
        return [f"{selector_dir.name}: case_study.json {case_error}"]
    if trace is None:
        return [f"{selector_dir.name}: sample_trace.json {trace_error}"]
    if case.get("schema_version") != "q-attention.q-ceasc-case-study.v2":
        errors.append(f"{selector_dir.name}: unsupported Case Study schema")
    if set(case.get("required_splits", [])) != REQUIRED_SPLITS:
        errors.append(f"{selector_dir.name}: Case Study split coverage is incomplete")
    if set(case.get("checkpoint_policy", [])) != REQUIRED_CHECKPOINTS:
        errors.append(f"{selector_dir.name}: Case Study checkpoint coverage is incomplete")
    if not REQUIRED_REPRESENTATIONS.issubset(set(case.get("representation_inventory", []))):
        errors.append(f"{selector_dir.name}: required tensor representations are missing")
    cases = case.get("cases")
    if not isinstance(cases, list) or not cases:
        errors.append(f"{selector_dir.name}: Case Study has no cases")
    else:
        if set(item.get("split") for item in cases if isinstance(item, dict)) != REQUIRED_SPLITS:
            errors.append(f"{selector_dir.name}: cases do not cover train/valid/test")
        if set(item.get("checkpoint") for item in cases if isinstance(item, dict)) != REQUIRED_CHECKPOINTS:
            errors.append(f"{selector_dir.name}: cases do not cover all checkpoints")
        for index, item in enumerate(cases):
            if not isinstance(item, dict):
                errors.append(f"{selector_dir.name}: case {index} is not an object")
                continue
            for field in ("sentence", "tokens", "token_ids", "subject", "object", "gold_relation", "baseline_prediction", "selector_prediction", "representations"):
                if field not in item:
                    errors.append(f"{selector_dir.name}: case {index} missing {field}")
    if trace.get("schema_version") != "sample-trace.v1":
        errors.append(f"{selector_dir.name}: sample trace schema is not sample-trace.v1")
    semantic = trace.get("semantic_contract")
    if not isinstance(semantic, dict) or set(semantic.get("required_splits", [])) != REQUIRED_SPLITS:
        errors.append(f"{selector_dir.name}: semantic split contract is incomplete")
    if not isinstance(semantic, dict) or set(semantic.get("required_checkpoints", [])) != REQUIRED_CHECKPOINTS:
        errors.append(f"{selector_dir.name}: semantic checkpoint contract is incomplete")
    if not isinstance(trace.get("sample_selection"), dict) or int(trace["sample_selection"].get("selected_count", 0)) != (len(cases) if isinstance(cases, list) else -1):
        errors.append(f"{selector_dir.name}: sample trace count disagrees with Case Study")
    experiment = trace.get("experiment")
    if not isinstance(experiment, dict) or experiment.get("config_sha256") != config_sha256:
        errors.append(f"{selector_dir.name}: sample trace config hash mismatch")
    for entry in case.get("tensor_manifest", []):
        if not isinstance(entry, dict):
            errors.append(f"{selector_dir.name}: malformed tensor manifest entry")
            continue
        target = selector_dir / str(entry.get("path", ""))
        if not target.is_file():
            errors.append(f"{selector_dir.name}: missing tensor capture {target.name}")
            continue
        import hashlib
        digest = hashlib.sha256(target.read_bytes()).hexdigest()
        if digest != entry.get("sha256"):
            errors.append(f"{selector_dir.name}: tensor hash mismatch for {target.name}")
    return errors
...
def _parse_args() -> argparse.Namespace:
    ...
    target.add_argument("--run-dir", metavar="RUN_DIR")
    target.add_argument(
        "--fresh",
        action="store_true",
        help="validate the checked-out Q-CEASC contract before creating a new run",
    )
...
def main() -> int:
    ...
    if args.fresh:
        ...
        errors = []
        if config is None:
            errors.append(f"config {config_path}: {config_error}")
        elif config.get("schema_version") != SUPPORTED_SCHEMA:
            errors.append("config has an unsupported Q-CEASC grouped counterfactual schema")
        elif config.get("selectors") != EXPECTED_SELECTORS:
            errors.append("config selectors must be disabled, grouped candidate, matched classical control, and seeded random control")
        elif config.get("structural_control") != "random_grouped_counterfactual":
            errors.append("config structural_control must be the seeded random grouped control")
        elif config.get("case_study", {}).get("records", {}).keys() != REQUIRED_SPLITS:
            errors.append("config case_study.records must declare train, valid, and test")
        elif set(config.get("case_study", {}).get("checkpoints", [])) != REQUIRED_CHECKPOINTS:
            errors.append("config case_study.checkpoints must declare initial, best-valid, and final")
        if runner is None:
            errors.append(str(runner_error))
        result = {
            "read_only": True,
            "mode": "fresh",
            ...
            "runner": RUNNER_NAME if runner is not None else None,
            "errors": errors,
        }
        if args.json:
            print(json.dumps(result, ensure_ascii=True, indent=2, sort_keys=True))
        else:
            print("Q-CEASC grouped counterfactual fresh-run preflight (READ ONLY)")
    ...
    config, config_error = _read_json(config_path)
    if config is None:
        result["errors"].append(f"config {config_path}: {config_error}")
    else:
        import hashlib
        ...
        for selector in selectors:
            if selector == "disabled":
                continue
            selector_dir = run_dir / "selectors" / selector
            if (selector_dir / "metrics.json").is_file():
                result["errors"].extend(_case_study_errors(selector_dir, config_sha256))
...
def _emit(result: dict[str, Any], json_only: bool, *, failure: bool) -> int:
    if json_only:
        ...
        print("Q-CEASC counterfactual resume diagnostic (READ ONLY)")
