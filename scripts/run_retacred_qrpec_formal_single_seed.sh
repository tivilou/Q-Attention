...
RUN_DIR=${RESUME_DIR:-${OUTPUT_DIR:-runs/retacred_qrpec_formal_single_seed/${STAMP}_seed13}}
...
[[ ${SKIP_PREFLIGHT} -eq 1 || ${DRY_RUN} -eq 1 ]] || bash scripts/check_retacred_qrpec_formal_single_seed.sh

COMMAND=(
  "${PYTHON_BIN}" experiments/run_retacred_qrpec_formal_single_seed.py
  --config configs/retacred_qrpec_formal_single_seed.json
  --device cuda
  --seed 13
...
EXPORT_COMMAND=(bash scripts/export_retacred_qrpec_formal_single_seed_report.sh --run-dir "${RUN_DIR}")
